...
def make_mlps(ob_space, ac_space, cfg):
    '''
    Create Sequential() keras model based on the input cfg dictionary
    '''
    ...
    # Wrap it around with Schulman's class
    policy = StochPolicyKeras(net, probtype)
    
    # Create Baseline value func (Keras) net
    vfnet = Sequential()
    # Add input and hidden layers
    for (i, layeroutsize) in enumerate(hid_sizes):
...
def make_filters(cfg, ob_space):
    '''
    Make state and reward normalisers (filter)
    Zfilter is just the norm scaler (e.g. sklearn.StandardScaler)
    with a running average of mean and std devs.
    '''
    if cfg["filter"]:
...
class AgentWithPolicy(object):
    '''
    Agent with Policy wrapper object
    '''
...
class TrpoAgent(AgentWithPolicy):
    ...
    def __init__(self, ob_space, ac_space, usercfg):
        cfg = update_default_config(self.options, usercfg)
        # make policy and value func (baseline) neural nets
        policy, self.baseline = make_mlps(ob_space, ac_space, cfg)
        # Add norm scalers for state and rewards (these are running averages 
        # of mean and std dev)
        obfilter, rewfilter = make_filters(cfg, ob_space)
        # The actual TRPO optimizer that accepts the policy network
