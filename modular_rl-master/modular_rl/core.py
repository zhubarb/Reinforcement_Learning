...
def compute_advantage(vf, paths, gamma, lam):
    '''
    vf is the value function net, created by make_mlps()
    paths is the list of rollout paths (dicts)
    '''
    # Loop through all paths and compute return, baseline, advantage
    for path in paths:
        # Return Q values: or rew = [1,1,1], returns [ 2.9701, 1.99, 1.]
        path["return"] = discount(path["reward"], gamma)
        # value function predictions
        b = path["baseline"] = vf.predict(path)
        #  append predictions with 0 or the lasta element of b
...
def run_policy_gradient_algorithm(env, agent, usercfg=None, callback=None):
    ...
    for _ in xrange(cfg["n_iter"]):
        ...
        paths = compute_advantage(agent.baseline, paths, gamma=cfg["gamma"], lam=cfg["lam"])
        # VF Update ========
...
def rollout(env, agent, timestep_limit):
    ...
    Return the path as a dictionary that contains:
    - actions
    - terminated flag
    - observations
    - rewards
    - prob outputs from the policy net
    """
    ob = env.reset()
    terminated = False
...
def do_rollouts_serial(env, agent, timestep_limit, n_timesteps, seed_iter):
    '''
    Do experience collection (rollout) for 'n_timesteps' experiences
    Returns a list of dictionaries (paths), where each path (dict) is returned by
    the rollout() method.
    '''
    paths = []
    timesteps_sofar = 0
    
    # Collect n_timesteps experiences
    while True:
...
class NnRegression(EzPickle):
    '''
    Wrapper around the Keras net (value function net) whose output
    is continuous, i.e. carries out regression
    '''
    def __init__(self, net, mixfrac=1.0, maxiter=25):
        ...
        self.predict = theano.function([x_nx], net.output, **FNOPTS)# compiled theano func
...
class NnVf(object):
    ...
    def predict(self, path):
        # append observation with time indicator, as a feature
        ob_no = self.preproc(path["observation"])
        # self.predict = theano.function([x_nx], net.output, **FNOPTS) function of the NnRegression class 
